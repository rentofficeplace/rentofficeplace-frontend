...
function App() {
  ...
  // New Authentic Modals Toggle States
  const [showAuthModal, setShowAuthModal] = useState(false); // image_5d411f.png
  const [showRecoveryModal, setShowRecoveryModal] = useState(false); // image_5d4803.png
  const [recoveryPhone, setRecoveryPhone] = useState("");
  const [dismissRecoveryForever, setDismissRecoveryForever] = useState(false);

  // Step 2 Property Attributes
  ...
  // Automated Drop-off Form Monitor Hook
  useEffect(() => {
    let timer;
    if (currentPage === "list-property-flow" && listingStep === 1 && !otpSent && !showAuthModal && !dismissRecoveryForever) {
      timer = setTimeout(() => {
        setShowRecoveryModal(true);
      }, 20000); // Prompts drop-off recovery after 20 seconds of inactivity
    }
  ...
  const triggerInitialSubmit = (e) => {
    e.preventDefault();
    setShowAuthModal(true); // Launches login/register popup modal instantly
  };
  ...
  const verifyAuthOTP = (e) => {
    ...
    if (otpCode === "1234") {
      ...
      setListingStep(2); // Safely unlocks page step 2 form fields
      setErrorMsg("");
    } else {
  ...
  return (
    <div style={{ backgroundColor: '#f5f7fa', minHeight: '100vh', position: 'relative' }}>
      ...
      <nav className="navbar navbar-expand-lg bg-dark navbar-dark py-3 sticky-top shadow-sm">
        <div className="container">
          ...
          <div className="ms-auto">
            <button className="btn btn-link text-white text-decoration-none me-3 fw-semibold" onClick={() => setShowAuthModal(true)}>Login / Register</button>
            {currentPage === "landing" ? (
              <button className="btn btn-warning fw-bold rounded-pill px-4 shadow" onClick={() => setCurrentPage("list-property-flow")}>Post Property FREE</button>
            ) : (
              <button className="btn btn-outline-light btn-sm fw-bold rounded-pill px-3" onClick={() => setCurrentPage("landing")}>← Back to Browse</button>
            )}
          </div>
        </div>
      </nav>

      {/* VIEW PANEL 1: USER LANDING SEARCH SEARCH INTERFACE */}
      {currentPage === "landing" && (
        <div>
      ...
      {/* ========================================================
          HIGH-FIDELITY LOGIN / REGISTER POPUP MODAL (IMAGE_5D411F)
          ======================================================== */}
      {showAuthModal && (
          ...
          <div className="card shadow border-0 p-4 bg-white rounded-4 position-relative text-start" style={{ maxWidth: '440px', width: '92%' }}>
            ...
            {!otpSent ? (
              <form onSubmit={handleAuthContinue}>
                {/* Styled Prefix/Number Box Layout */}
                <div className="mb-4 p-2 rounded-3 border d-flex align-items-center" style={{ backgroundColor: '#fff', borderColor: '#4a90e2', borderWidth: '1.5px' }}>
                  <div className="pe-2 border-end d-flex align-items-center">
                    <label className="position-absolute small fw-bold text-primary px-1" style={{ top: '80px', fontSize: '0.72rem', background: '#fff' }}>Phone Number</label>
                  ...
                  <input 
                    type="number" 
                    className="form-control border-0 bg-transparent shadow-none fs-5 ps-3 py-1" 
                    placeholder=" "
                    required 
                ...
                <button type="submit" className="btn w-100 fw-bold py-2 text-white mb-3 shadow-sm rounded-3 transition" style={{ backgroundColor: '#6cbdff', borderColor: '#6cbdff', fontSize: '1.1rem' }}>
                  Continue
                </button>
              </form>
            ) : (
              <form onSubmit={verifyAuthOTP}>
                ...
                <button type="submit" className="btn btn-success w-100 fw-bold py-2 rounded-3 text-white mb-3" style={{ fontSize: '1.1rem' }}>
                  Verify & Validate
                </button>
              </form>
            )}

            {/* Login with Email Row */}
            <button type="button" className="btn btn-outline-dark w-100 py-2 rounded-3 d-flex align-items-center justify-content-center fw-bold mb-4" style={{ borderColor: '#d3d3d3', color: '#092143', fontSize: '1.05rem' }}>
              <span className="me-2">✉</span> Login with Email
            </button>

            {/* Compliance Footer Disclaimer */}
            <div className="text-center text-muted border-top pt-3" style={{ fontSize: '0.72rem' }}>
              By clicking you agree to <a href="#terms" className="text-primary text-decoration-none fw-semibold">Terms and Conditions</a>
            </div>
      ...
      {/* ========================================================
          DROP-OFF RECOVERY SYSTEM BACKDROP PANEL (IMAGE_5D4803)
          ======================================================== */}
      {showRecoveryModal && (
        <div className="modal-backdrop fade show" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 1050, display: 'flex', alignItems: 'center', justifyindex: 'center', justifyContent: 'center' }}>
          <div className="card shadow-lg border-0 p-4 bg-white rounded-3 position-relative text-center" style={{ maxWidth: '420px', width: '90%' }}>
            ...
            <p className="small mt-2 mb-4">
              Now post via <span style={{color: '#25D366'}}>🟢 Whatsapp</span>. Send a "hi" to <br />
              <a href="https://wa.me/917428197035" target="_blank" rel="noreferrer" className="fw-bold text-primary text-decoration-none">7428197035</a> or <a href="https://wa.me/917428197035" target="_blank" rel="noreferrer" className="fw-bold text-primary">click here</a>
            </p>
